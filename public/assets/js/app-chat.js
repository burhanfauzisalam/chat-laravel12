...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const filterChatContacts = (selector, searchValue, placeholderSelector) => {
    ...
    items.forEach(item => {
      const nameAttr = item.getAttribute('data-room-name') || '';
      const topicAttr = item.getAttribute('data-room-topic') || '';
      const haystack = `${nameAttr} ${topicAttr} ${item.textContent}`.toLowerCase();
    ...
    if (placeholderEl) {
      placeholderEl.classList.toggle('d-none', visibleCount > 0);
    }
  };
  ...
  if (elements.chatSidebarLeftUserAbout && elements.textareaInfo) {
    const maxLength = parseInt(elements.chatSidebarLeftUserAbout.getAttribute('maxlength') || '0', 10) || 0;
  ...
  // Attach message send event (disabled for MQTT chat which handles sending itself)
  if (elements.formSendMessage && !elements.formSendMessage.dataset.disableDefaultSend) {
